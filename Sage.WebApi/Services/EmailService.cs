...
)
{
    ...
    public async Task SendEmailAsync<T>(string to, string subject, string view, T model, CancellationToken ct)
    {
        ...
        var res = await resend.EmailSendAsync(new EmailMessage
        {
            ...
            To = to,
            Subject = subject,
            HtmlBody = await RenderEmailAsync(view, model)
        }, ct);
    ...
    private async Task<string> RenderEmailAsync<T>(string view, T model)
    {
